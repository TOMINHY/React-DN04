  ...
  renderCart = () => {
    return this.props.gioHang.map((spGH) => {
      return <tr key={`cart-${spGH.maSP}`}>
        <td>{spGH.maSP}</td>
        <td>
          <img style={{ width: '25px' }} src={spGH.hinhAnh} alt="" />
        </td>
        <td>
          {spGH.tenSP}
        </td>
...
const mapStateToProps = (rootReducer) => {
  //lưu vào state vào  props của componemt
  return {
    gioHang: rootReducer.gioHangReducer
  }
}

//connect tạo ta 1 loại component mới bao gồm tính năng của Class component react + Redux
// mapDispatchToProps phải truyền vào vị trí thứ 2 của connect (nằm sau mapStateToProps)
//! nếu không có mapStateToProps thì tham số đầu để là null => (null,mapDispatchToProps)
